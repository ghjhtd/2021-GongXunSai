...
//	servo_control(1,-90,2000);
	...
	while(1)
	{
		if(USART3_RX_STA&0x8000)//可以放在定时器中断里执行
		{
			...
			for(t=0;t<len;t++)
			{
				 OPENMV[t]=USART3_RX_BUF[t];				
			}
			OPENMV[t]='\0';
			USART3_RX_STA=0;
			if(OPENMV[0] == 'q'){
				printf("Q%c%c%c%c%c%c",OPENMV[1],OPENMV[2],OPENMV[3],OPENMV[5],OPENMV[6],OPENMV[7]);
			}
			if(OPENMV[0] == 'C'){
				printf("C%c",OPENMV[1]);
			}
			...
			if(COMMAND[0]=='D')  //让舵机做相应的动作
			{
				...
				switch(witch){
					...
					case 1: //原料区下层
						...
						break;
					case 2:  //粗加工 绿
						servo_control(0,-65,100);
						delay_ms(600);
					servo_control(1,60,100);
						delay_ms(600);
						servo_control(4,5,100);
						delay_ms(600);
						
						servo_control(2,83,100);
						delay_ms(600);
