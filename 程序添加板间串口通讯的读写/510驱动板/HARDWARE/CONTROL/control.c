...
int MoveX_T(int x,int count,int zhongzhi_x){
		PS2_LX = x;
		PS2_LY = 0;
		TURN = 0; 
		bias = (zuo - you);
		...
		if(myabs(bias) < threshold && myabs(zuo-zhongzhi_x) > 156&& myabs(bias) < 10000){
			PS2_LY = -(zuo-zhongzhi_x) / 7; 
		}
		if(myabs(qian) < 80) {tempY1 = 1;}
		if(tempY1 == 1 && qian >200 && count_Biaozhi == 1 ) {temp_Dot--;tempY1 = 0;count_Biaozhi =0;}
		if(tempY1 == 1 && qian < -200 && count_Biaozhi == 1) {temp_Dot++;tempY1 = 0;count_Biaozhi =0;}
		if(temp_Dot == count&& count!=0) {temp_Dot=0;tempY1 = 0;PS2_LX = 0;PS2_LY = 0;TURN = 0;MOVEX_ENABLE=0;  return 0;}
		else return 1;
}        //在x轴上以速度x移动time ms
...
int MoveX(int x,int count,int zhongzhi_x){
	  //LED1 = !LED1;
		PS2_LX = x;
		PS2_LY = 0;
		TURN = 0; 
		bias = (zuo - you);
		count_Jishu++;
		if(count_Jishu > 100){count_Biaozhi = 1;count_Jishu = 0;LED0 = !LED0; }
		if(myabs(bias) > threshold && myabs(bias) < 5000){
			TURN = bias/18; 
		}
		if(myabs(bias) < threshold && myabs(zuo-zhongzhi_x) > 156 && myabs(bias) < 10000){
		...
		if(tempY1 == 1 && PS2_LX < -10 && count_Biaozhi == 1 && myabs(zuo) != 30000 ) {temp_Dot--;tempY1 = 0;count_Biaozhi =0;}
		if(tempY1 == 1&& PS2_LX > 10 && count_Biaozhi == 1 && myabs(zuo) != 30000 ) {temp_Dot++;tempY1 = 0;count_Biaozhi =0;}
		if(temp_Dot == count && count!=0) {temp_Dot=0;tempY1 = 0;PS2_LX = 0;PS2_LY = 0;TURN = 0;MOVEX_ENABLE=0; return 0;}
		else return 1;	
...
int MoveY(int y,int count,int zhongzhi_y){
		PS2_LX = 0;
		PS2_LY =y;
		TURN = 0; 
		bias = qian-zhongzhi_y;
		if(jishu_shineng==1)count_JishuY++;
	  if(count_JishuY > 50){count_BiaozhiY = 1;count_JishuY = 0;jishu_shineng=0;LED0 = !LED0;}
		if(myabs(bias) > threshold && myabs(bias) < 10000){
			PS2_LX = bias / 9; 
		}
		//if(myabs(zuo) < 211) {tempY2 = 1;}
		if(tempY2 == 1 && myabs(zuo) <200&&count_BiaozhiY == 1) {jishu_shineng=1;temp_DotY++;tempY2 = 0;count_BiaozhiY =0;}
		else if(myabs(zuo) >=200) tempY2 = 1;
			
		//if(tempY2 == 1 && myabs(zuo) <200&& PS2_LY < -20 && count_BiaozhiY == 1) {temp_DotY++;tempY2 = 0;count_Biaozhi =0;}
		
		if(temp_DotY == count && count!=0){temp_DotY=0;tempY2 = 0;PS2_LX = 0;PS2_LY = 0;TURN = 0;MOVEY_ENABLE=0;  return 0;}
		else return 1;	
