...
//		      		PCout(3)=0;
		      		if(PS2_KEY==PSB_L1)
		      			TURN=128;
							...
							if(PS2_KEY2==PSB_BLUE)
							{
								servo_angle=-5;
								printf("M%d%d\r\n",servo_num, servo_angle);
							}
							if(PS2_KEY2==PSB_GREEN)
							{
								servo_angle=5;
								printf("M%d%d\r\n",servo_num, servo_angle);
							}
...
//		      		printf("%d \r\nPS2_KEY:",PS2_KEY);
		      		...
		      		if(ADC_Key(ADCConvertedValue[0][2],1) == 3 || PS2_Keyscan(1)==PSB_SELECT)      //按键0返回主菜单
		      			{	
		      				OLED_Clear();
		      				program=0;
		      				break;
