...
extern u8 Flag_Stop,delay_50,delay_flag,uart_show;         //停止标志位 50ms精准演示标志位(上位机使用)
extern int Position_Zero;            //编码器的脉冲计数
extern int key;                                  //电机PWM变量 应是Motor的 向Moto致敬	
...
extern int AIM_max;//速度挡位

extern int volatile qian,zuo,you,Xunxian,Pinxin,Juli,Chuizhi;
extern int X,Y;   //小车在地图上的定位坐标，地图左下角的坐标是0，0
extern int threshold;   //巡线阈值
extern int Mode;
extern int num[8],num_cnt,cishu_flag;
extern int bias,temp_Dot,tempY1,temp_Turn,temp_Sudu,count_Jishu,count_Biaozhi,count_JishuY,count_BiaozhiY,temp_DotY,tempY2;
